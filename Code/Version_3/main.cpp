...
void allocateGrid() {
	...
	for (int i=0; i<numRows; i++){
		...
		tileLock[i] = (pthread_mutex_t*) malloc(numCols * sizeof(pthread_mutex_t));
	}
...
void initializeLocations() {
	...
	for(int i = 0; i < numBoxes; i++) {
		...
		//lock spot
		pthread_mutex_lock(&tileLock[roboCoords.first][roboCoords.second]);
		...
		//lock spot
		pthread_mutex_lock(&tileLock[boxCoords.first][boxCoords.second]);
...
void initializeApplication(void)
{
	...
	for (int k=0; k<numRows;k++) {
		for (int j=0; j< numCols; j++) {
			pthread_mutex_init(&tileLock[k][j], NULL);
		}
	}
